...
module Transactions
  class OfxImportService < ApplicationService
    ...
    validate :file_content_size

    def file_content_size
      return if file_content.blank?

      errors.add(:file_content, "is too large (max 10MB)") if file_content.bytesize > MAX_FILE_SIZE
    end
    ...
    def call
      ...
      # Batch-load existing fit_ids for this account to avoid N+1 duplicate checks
      existing_fit_ids = household.transactions
        .where(account: account)
        .where("metadata ? 'ofx_fit_id'")
        .pluck(Arel.sql("metadata->>'ofx_fit_id'"))
        .to_set
      ...
      ActiveRecord::Base.transaction do
        parsed[:transactions].each_with_index do |txn, idx|
          ...
          if fit_id.present?
            existing = existing_fit_ids.include?(fit_id)
          else
      ...
      statement_import&.update(status: "failed", errors_log: [{ error: e.message }])
