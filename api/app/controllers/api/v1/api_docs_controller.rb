    ...
    class ApiDocsController < ApplicationController
      ...
      API_DOCS_HTML = <<~HTML
        ...
        <body>
          ...
          <div class="auth-note">
            <strong>🔑 Authentication:</strong> All endpoints require an <code>X-API-Key</code> header. Generate API keys in Settings.
          </div>

          <div class="rate-note">
            <strong>⏱ Rate Limiting:</strong> 60 requests per minute per API key. Responses include <code>X-RateLimit-Limit</code> and <code>X-RateLimit-Remaining</code> headers.
          </div>

          <h2>Accounts</h2>
          <div class="endpoint">
            <span class="method">GET</span> <span class="path">/api/v1/accounts</span>
            <p class="desc">List all visible accounts with current balances.</p>
          </div>
          ...
          <div class="endpoint">
            ...
            <p class="desc">Public endpoint (no API key required). Returns net worth data for embedding. Requires a share token.</p>
          </div>
          <div class="endpoint">
            <span class="method">GET</span> <span class="path">/api/v1/embed/spending?token=TOKEN</span>
            <p class="desc">Public endpoint (no API key required). Returns monthly spending data for embedding. Requires a share token.</p>
          </div>
        </body>
        </html>
      HTML
