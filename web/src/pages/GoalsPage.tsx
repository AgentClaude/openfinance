        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {displayGoals.map((goal) => (
            <Card key={goal.id} className="p-5">
              ...
              {/* Milestone markers */}
              {(goal.milestones.length > 0 || goal.nextMilestonePercentage) && (
                <div className="flex items-center gap-1 mb-2">
                  {[25, 50, 75, 100].map((pct) => {
                    const achieved = goal.milestones.find(m => m.percentage === pct);
                    const isNext = goal.nextMilestonePercentage === pct;
                    return (
                      <div
                        key={pct}
                        className={`flex items-center justify-center w-8 h-8 rounded-full text-xs font-medium transition-all ${
                          achieved
                            ...
                            : isNext
                            ? 'bg-gray-100 dark:bg-gray-700 text-gray-500 dark:text-gray-400 ring-1 ring-dashed ring-gray-300 dark:ring-gray-600'
                            : 'bg-gray-50 dark:bg-gray-800 text-gray-300 dark:text-gray-600'
                        }`}
                        title={achieved ? `${achieved.emoji} ${achieved.label} — ${formatCurrency(achieved.amountAtMilestone)}` : `${pct}% milestone`}
                      >
                        {achieved ? achieved.emoji : `${pct}%`}
                      </div>
                    );
                  })}
                </div>
